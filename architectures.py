...
def get_architecture(arch: str, dataset: str, pytorch_pretrained: bool=False) -> torch.nn.Module:
    ...
    elif arch == "resnet50" and dataset in ["pneumonia", "breakhis", "isic", "hyper"]:
        model = resnet50(pretrained=pytorch_pretrained)
        model.fc = nn.Linear(2048, get_num_classes(dataset))
        model = model.cuda()
